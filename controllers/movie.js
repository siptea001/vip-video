...
exports.doSearchMovie = function (req, res, next) {
    ...
    if (searchMoviename.startsWith('http')) {
        ...
        searchMoviename = 'https://jx.aidouer.net/?url=' + searchMoviename;
        ...
        if (movies.length === 0) {
            Movie.getMovieByName(searchMoviename, function (err, result) {
                ...
                result.forEach(function (element) {
                    ...
                    if(element.info !== undefined && element.info !== null && element.info.toString().length > 0) {
                        element.description = element.info.toString().substring(0, 30);
                    }
                    element.createTime = moment().format('YYYY-MM-DD');
                })
            ...
            movies.forEach(function (element) {
                let url = element.url;
                element.url = url.substring(url.lastIndexOf('/') + 1)
                if (element.description === undefined || element.description === null || element.description === '') {
                    element.description = element.title;
                }
                if (element.createTime === undefined || element.createTime === null || element.createTime === '') {
                    element.createTime = new Date().getDate();
                }
                element.img = 'https:' + element.img;
