...
exports.searchMovie = function (keyword, callback) {
    ...
    https.get(option1, function (res) {
        ...
        res.on('end', function () {
            ...
            let results = $('.result-right');
            let imgs = $('.qy-mod-cover');
            let infos = $('.qy-search-result-tit>.main-tit');
            let createTimes = $('.qy-search-result-info.half>.info-des');
            let descrts = $('.qy-search-result-info.multiple>.info-des');
            let res = [];
            results.each(function (index) {
                let title = $(infos[index]).attr('title');  // 标题
                let href = $(infos[index]).attr('href');     // 链接
                let img = $(imgs[index]).attr('src');        // 缩略图
                let createTime = $(createTimes[index]).attr('title'); // 创建时间
                let description = $(descrts[index]).attr('title'); // 简介
                if (res.length < 3) {
                    res.push({
                        title: title,
                        url: href,
